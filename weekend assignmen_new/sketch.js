...
let walkXPos, walkYPos, walkSpeed = 1, walkScale = 0.5; 
let walkFrame = 0, lastMoveTime = 0, frameDelay = 350;
let roadX, roadY, roadWidth, roadHeight; 
let showRoad = false;
let trafficSound;
...
let trafficPlaying = false;
let earphonePlaying = false;
...
function setup() {
  ...
  // Road boundary
  let roadRatio = road.width / road.height; 
...
function showOfficeScene() {
  ...
  // Play call sound once when ellipses visible
  if (currentOffice == 0 && ellipsesVisible && !callPlaying) {
    callSound.loop();
...
function mousePressed() {
  ...
  if (headphoneStart && !wearingHeadphone) {
    ...
    if (mouseX > width / 2 - imgW / 2 &&
      mouseX < width / 2 + imgW / 2 &&
      mouseY > height / 2 - imgH / 2 &&
      mouseY < height / 2 + imgH / 2) {
...
////////////////////////////////////////////////////////////////
function keyPressed() {
  if (keyCode == RIGHT_ARROW && showRoad && !showCity) {
    if (!trafficPlaying) {
      trafficSound.loop();
